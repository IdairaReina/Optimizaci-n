...
//Método de recorrido en amplitud (No funcional sin clase COLA)
void GRAFO::bfs(unsigned i, vector<bool>& visitado)
{
	//Desde la versión entregada en las actividades del tema 6, se ha corregido:
	//Un problema según el cual, si cada nodo adyacente al inicial tenía adyacencias exclusivas, la distancia aumentaría por error por cada nodo de primera generación examinado
	Queue cola(n);
...
//Función alternativa de Kruskal para dar detalles de componentes separadas en grafos no conexos
void GRAFO::Kruskal2(bool traza)
{
	...
	//Algoritmo muy poco óptimo para extraer la lista de aristas pesadas de manera ordenada en un principio
	for (unsigned i = 0; i < m; i++)
	{
		visitado[2 * m] = false;
...
void GRAFO::Prim(bool traza, unsigned start)
{
	...
	{
		M[i].inside = false;
		//2147483647 es el número máximo de un int de 4 bytes 
	...
	do {
		...
		//Bucle para imprimir las listas de predecesor, distancia y si están en M
		/*for (unsigned i = 0; i < n; i++)
...
void GRAFO::Dijkstra()
{
	...
	do
	{
		...
		for (unsigned i = 0; i < n; i++)
			if (!PermanentementeEtiquetado[i])
			{
				if (PermanentementeEtiquetado[candidato])
					...
					all = true; // Detecta si el candidato no es el último en salir
				if (d[i] < d[candidato])
					candidato = i;
			}
		...
		{
			//Se comprueba que el candidato tenga predecesor, ya que si le sumas al valor más alto de int, se pasan a negativo, es decir, se acepta su "mejora" y evitan que se detecte como inf.
...
//Método para saber si un nodo forma parte de un circuito (devuelve true si s pertenece a un circuito y false si no hay circuitos o s no pertenece a uno
...
//Método para todos los nodos pertenecientes al circuito de un nodo (Solo funciona si hay circuitos con ese nodo)
...
//Versión de mostrar camino donde la lista de predecesores usa índices de 1 a n.
...
void GRAFO::Floyd_Marshall(bool traza, bool tabla)
{
	...
	//Bucle de detección y actualización para circuitos negativos
	for (unsigned i = 0; i < n; i++)
		if (neg[i])
		{
			...
			{
				if (temp[j])
					neg[j] = false; // Ya que los circuitos actúan como un nodo en sí mismo, se desmarcan los nodos pertenecientes al circuito ya detectado
				if (pred[i][j] != 0)
			...
			for(unsigned j=0;j< n;j++)
				if(j!=i && pred[j][i] !=0) // Nodos con acceso o pertenecientes al circuito copian solo las mejoras que este implementa y el circuito en sí
					for (unsigned k = 0; k < n; k++)
					{
						if (temp[k])
							pred[j][k] = pred[i][k];
